...
CONVERSION_MODES = {"commercial", "relationship-only"}
...
LEAD_TEMPLATE_PATH = os.path.join(PROJECT_ROOT, "templates", "lead-template.md")
...
def validate_conversion_mode(value):
    if value not in CONVERSION_MODES:
        raise ValueError(f"Invalid conversion mode '{value}'. Expected one of: {', '.join(sorted(CONVERSION_MODES))}")
    return value
...
def validate_ready_for_conversion(frontmatter):
    ...
    if frontmatter.get("status") != "qualified":
...
def create_contact_record(person_name, account_name, lead_frontmatter, source_lead, account_path=None):
    ...
    if account_path:
        ...
        account_link = link_for("Accounts", slugify(account_name))
    frontmatter = {
        "id": slug,
...
def move_open_tasks(lead_link, account_link="", contact_link="", opportunity_link="", new_primary_type="contact", new_primary_link=""):
    moved = []
    ...
    for file_path in iter_markdown_files(TASKS_DIR):
        ...
        frontmatter["date-modified"] = date.today().strftime("%Y-%m-%d")
        if linked_to_primary_lead and new_primary_link:
            frontmatter["primary-parent-type"] = new_primary_type
            frontmatter["primary-parent"] = new_primary_link
        write_frontmatter_file(file_path, frontmatter, body)
...
def cmd_convert(args):
    ensure_core_dirs()
    conversion_mode = validate_conversion_mode(args.conversion_mode)
    if conversion_mode == "commercial" and args.autonomous and not args.opportunity_name:
        raise ValueError("Autonomous commercial conversion requires --opportunity-name so the specific opportunity is explicit.")
    ...
    contact_path = create_contact_record(
        person_name,
        company_name if conversion_mode == "commercial" else "",
        lead_frontmatter=frontmatter,
        source_lead=lead_link,
    ...
    if conversion_mode == "commercial":
        ...
        moved_tasks = move_open_tasks(
            lead_link,
            account_link=account_link,
            contact_link=contact_link,
            opportunity_link=opportunity_link,
            new_primary_type="opportunity",
            new_primary_link=opportunity_link,
        )
    else:
        copied_notes = copy_linked_records(
            NOTES_DIR,
        ...
        moved_tasks = move_open_tasks(
            lead_link,
            contact_link=contact_link,
            new_primary_type="contact",
            new_primary_link=contact_link,
        )
...
def build_parser():
    ...
    convert_parser = subparsers.add_parser("convert", help="Convert a qualified lead into durable CRM records.")
    convert_parser.add_argument("lead")
    convert_parser.add_argument("--opportunity-name")
    convert_parser.add_argument("--conversion-mode", default="commercial", choices=sorted(CONVERSION_MODES))
