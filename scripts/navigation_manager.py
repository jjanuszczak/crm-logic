...
ENTITY_ORDER = [
    ...
    ("Source-Artifacts", "Source Artifact"),
    ("Retainers", "Retainer"),
    ("Invoices", "Invoice"),
    ("Payments", "Payment"),
    ("Activities", "Activity"),
...
def record_title(record):
    ...
    candidates = {
        ...
        "Tasks": ["task-name"],
        "Inbox": ["title"],
        "Deal-Flow": ["startup-name", "deal-name", "name", "title"],
...
def record_summary(record):
    ...
    summary = {
        ...
        "Engagements": summary_from_parts(
            frontmatter.get("engagement-type"),
            stage_phrase(frontmatter.get("status"), "status"),
            linked_phrase(frontmatter.get("organization") or frontmatter.get("account"), "for", record["crm_data_path"]),
        ),
        "Workstreams": summary_from_parts(
            frontmatter.get("workstream-type"),
            stage_phrase(frontmatter.get("status"), "status"),
            linked_phrase(frontmatter.get("engagement"), "under", record["crm_data_path"]),
        ),
        "Source-Artifacts": summary_from_parts(
            source_phrase(frontmatter.get("source-system")),
            frontmatter.get("source-type"),
            linked_phrase(frontmatter.get("primary-parent"), "for", record["crm_data_path"]),
        ),
        "Retainers": summary_from_parts(
            stage_phrase(frontmatter.get("status"), "status"),
            frontmatter.get("cadence"),
            linked_phrase(frontmatter.get("engagement"), "for", record["crm_data_path"]),
        ),
        "Invoices": summary_from_parts(
            stage_phrase(frontmatter.get("status"), "status"),
            due_phrase(frontmatter.get("due-date")),
            linked_phrase(frontmatter.get("engagement"), "for", record["crm_data_path"]),
        ),
        "Payments": summary_from_parts(
            stage_phrase(frontmatter.get("status"), "status"),
            frontmatter.get("received-date"),
            linked_phrase(frontmatter.get("invoice"), "against", record["crm_data_path"]),
        ),
...
def record_meta(record):
    ...
    field_map = {
        ...
        "Engagements": ["owner", "engagement-type", "status", "commercial-model", "currency", "contracted-value"],
        "Workstreams": ["owner", "workstream-type", "status", "priority", "target-end-date"],
        "Source-Artifacts": ["owner", "source-system", "source-type", "confidentiality", "status", "last-reviewed"],
        "Retainers": ["owner", "status", "currency", "amount", "cadence", "period-start", "period-end"],
        "Invoices": ["owner", "status", "currency", "amount", "issue-date", "due-date"],
        "Payments": ["owner", "status", "currency", "amount", "received-date", "payment-method"],
        "Notes": ["owner", "note-type", "reuse-classification", "date-created", "date-modified"],
        "Tasks": ["owner", "status", "priority", "due-date", "engagement", "workstream"],
        "Inbox": ["owner", "status", "source", "captured-at", "date-created"],
        "Deal-Flow": ["owner", "fundraising-stage", "coverage-status", "date-sourced"],
    }
    items = []
